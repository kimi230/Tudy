...
export default function Category() {
  ...
  if (!category) {
    return (
      <div className="text-center py-20 text-gray-400">카테고리를 불러오는 중...</div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link to="/" className="text-sm text-indigo-600 hover:underline">← 홈으로</Link>
      ...
      {videos.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm">
            <Link to="/request" className="text-indigo-600 hover:underline">영상을 신청</Link>해보세요!
          </p>
        </div>
      ) : (
