...
CATEGORY_CHANNELS: dict[str, list[str]] = {
    ...
    "business": ["Harvard Business Review", "Stanford Graduate School of Business", "Talks at Google",
                 "Economics Explained", "The Economist", "Y Combinator", "a]16z",
    ...
    "motivation": ["Goalcast", "Motivation2Study", "After Skool", "Einzelgänger",
                   "Academy of Ideas"],
    ...
    "anime": ["Gigguk", "Mother's Basement", "Super Eyepatch Wolf",
              "Nux Taku", "Trash Taste"],
    "pronunciation": ["Rachel's English", "English with Lucy", "Speak English With Vanessa",
                      "Pronunciation with Emma", "mmmEnglish"],
    "daily": ["English with Lucy", "BBC Learning English", "engVid",
              "English Addict with Mr Duncan"],
    "travel": ["Wolters World", "Rick Steves' Europe", "Geography Now",
               "Kara and Nate"],
}
...
LEGENDARY_SPEAKERS = {
    ...
    "Shawn Achor", "James Clear", "Cal Newport", "Robert Waldinger",
    "Carol Dweck", "Daniel Kahneman", "Steven Pinker", "Rutger Bregman",
    # Public figures
    "Bill Gates", "Elon Musk", "Barack Obama", "Jordan Peterson",
    # YouTube educators
    "Lex Fridman", "Andrew Huberman", "Grant Sanderson", "Destin Sandlin",
    "Michael Stevens", "Sam Wendover", "Derek Muller", "CGP Grey",
    "Hank Green", "John Green", "Ali Abdaal", "Mark Rober",
}
...
CATEGORY_KEYWORDS: dict[str, list[str]] = {
    "pronunciation": ["pronunciation", "English speaking", "accent training", "phonics"],
    "business": ["business English", "leadership", "management", "economics", "global economy", "startup"],
    "daily": ["daily conversation", "everyday English", "life advice", "life lessons"],
    "academic": ["lecture", "university", "academic English", "deep interview", "podcast"],
    "travel": ["travel", "culture", "adventure", "travel English"],
    "tech": ["technology", "programming", "innovation", "engineering", "artificial intelligence"],
    "current": ["current events", "world affairs", "logistics", "geopolitics", "global trends"],
    "culture": ["culture", "society", "history", "history animated", "documentary"],
    "science": ["science", "research", "discovery", "math", "physics", "engineering explained"],
    "motivation": ["motivation", "inspiration", "personal growth", "success", "mindset"],
    "news": ["breaking news", "news analysis", "news English", "world news", "news report"],
    "anime": ["animation", "animated series", "voice acting English", "cartoon analysis"],
}
...
ZH_CATEGORY_CHANNELS: dict[str, list[str]] = {
    "daily": ["TED中文", "一席YiXi", "混知", "老高与小茉"],
    "business": ["财经冷眼", "李永乐老师", "半佛仙人", "小Lin说"],
    "culture": ["李子柒", "国家地理", "CCTV纪录"],
    "news": ["CCTV中文国际", "凤凰卫视", "观察者网"],
    "tech": ["老石谈芯", "林亦LYi", "极客湾", "影视飓风", "何同学"],
    "science": ["妈咪说", "柴知道", "李永乐老师", "老高与小茉"],
    "academic": ["罗翔说刑法"],
}
...
JA_CATEGORY_CHANNELS: dict[str, list[str]] = {
    "daily": ["日本語の森", "三本塾", "もしもしゆうすけ", "Comprehensible Japanese"],
    "business": ["テレ東BIZ", "NewsPicks", "中田敦彦のYouTube大学",
                 "両学長 リベラルアーツ大学", "PIVOT 公式チャンネル"],
    "culture": ["NHK", "Japan Inside", "テレビ東京", "QuizKnock"],
    "news": ["ANNnewsCH", "TBS NEWS DIG", "日テレNEWS"],
    "tech": ["キオクシア", "CNET Japan", "ITmedia"],
    "science": ["予備校のノリで学ぶ", "GENKI LABO", "科学はすべてを解決する", "VAIENCE バイエンス"],
}
...
EN_CATEGORY_INTENTS: dict[str, list[str]] = {
    "pronunciation": ["shadowing", "listening practice", "accent drill", "pronunciation lesson",
                      "speaking fluently", "native speaker tips"],
    "business": ["case study", "leadership talk", "management lecture", "workplace communication",
                 "economics explained", "global economy analysis", "entrepreneurship"],
    "daily": ["conversation practice", "real life English", "communication tips", "storytelling",
              "social skills", "relationship advice"],
    "academic": ["university lecture", "research talk", "seminar", "critical thinking",
                 "deep interview", "expert conversation", "book review"],
    "travel": ["travel guide", "travel conversation", "culture guide", "trip tips",
               "travel vlog", "living abroad"],
    "tech": ["explained", "deep dive", "engineering talk", "product analysis",
             "AI explained", "future of technology"],
    "current": ["current affairs", "policy explain", "global issues",
                "logistics explained", "geopolitical analysis", "economic trends"],
    "culture": ["history explained", "culture documentary", "society talk", "art discussion",
                "history animated", "investigative journalism", "philosophy"],
    "science": ["science explained", "documentary", "experiment", "scientific talk",
                "math visualization", "engineering explained", "space exploration"],
    "motivation": ["motivational speech", "self improvement", "growth mindset", "life lessons",
                   "habits", "productivity tips", "resilience"],
    "news": ["news analysis", "breaking news explained", "news commentary", "media literacy",
             "weekly news roundup", "journalism"],
    "anime": ["animation analysis", "voice acting", "animated storytelling",
              "cartoon review", "animation explained"],
}
...
CATEGORY_FILTER_PROFILES: dict[str, dict[str, dict[str, Any]]] = {
    "en": {
        ...
        "current": {"min_views": 8000, "min_quality_score": 0.30, "min_relevance_score": 0.18},
        "news": {"min_views": 5000, "min_quality_score": 0.35, "min_relevance_score": 0.22},
...
def _compute_recency_score(upload_date: str, category: str, language: str) -> float:
    ...
    # News = breaking/time-critical, decays fast
    if category == "news":
        if age_days <= 3:
            return 1.0
        if age_days <= 14:
    ...
    # Current affairs = analysis/essays, longer shelf life
    if category == "current":
        if age_days <= 14:
            return 1.0
        if age_days <= 60:
...
def prefilter(
    ...
    for r in results:
        ...
        # Content quality score from lexical signals + source credibility + engagement (0-1)
        quality_score = 0.5 + (good_hits * 0.12) - (bad_hits * 0.22)
        if is_curated:
            quality_score += 0.12
        # Engagement boost: like/view ratio as quality proxy
        likes = r.get("like_count", 0)
        if views > 0 and likes > 0:
            like_ratio = likes / views
            if like_ratio >= 0.04:        # top-tier engagement
                quality_score += 0.10
            elif like_ratio >= 0.025:     # good engagement
                quality_score += 0.05
...
def quick_analysis(
    ...
    for i, cand in enumerate(sorted_candidates):
        ...
        # Subtitle coherence: penalize low-quality auto-generated subtitles
        ...
        if language == "en":
            ...
            if sentences:
                ...
                # Very short avg (<3 words) = choppy/bad subs; very long (>40) = no punctuation
                if avg_sentence_words < 3 or avg_sentence_words > 40:
                    content_quality_score = max(0, content_quality_score - 0.15)
            # Penalize [Music]/[Applause] heavy subtitles (auto-generated noise)
            noise_markers = subtitle_text.lower().count("[music]") + subtitle_text.lower().count("[applause]")
            if noise_markers > 5:
                content_quality_score = max(0, content_quality_score - 0.10)
...
""",
    ...
    parser.add_argument(
        "--level",
        type=str,
        choices=["beginner", "elementary", "intermediate", "upper-intermediate", "advanced"],
